...
func SendJSONMetrics(m ClientMetrics, reportInterval int, host string) {
	...
	for {
		for k, v := range m.GetMetrics() {
			...
			log.Println("metric/internal/client/util/util.go SendMetrics: ", req, "; ", "post status", resp.Status)
			resp.Body.Close()
		}
...
func SendBatchJSONMetrics(m ClientMetrics, reportInterval int, host string) {
	...
	for {
		...
		log.Println("metric/internal/client/util/util.go SendBatchMetrics: ", metricsBatch, "; ", "post status", resp.Status)
		resp.Body.Close()
